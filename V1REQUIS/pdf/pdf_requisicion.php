...
if (isset($_GET["ref"])){
    $id=$_GET['ref'];
    $datos=$conectar->query("SELECT usuarios.*, solicitudes.*, departamentos.* FROM solicitudes 
    INNER JOIN usuarios ON solicitudes.id_user= usuarios.id_user
    INNER JOIN departamentos ON usuarios.departamento_id = departamentos.id_depto WHERE MD5(concat('".$key."',id_folio))='".$id."'");
...
function Footer()
{
   ...
   //Creación del objeto de la clase heredada
    ...
    $pdf->SetFont('Arial','B',14);   
    $pdf->Cell(22,7,utf8_decode('Nombre: '), 0, 0,'L');
    ...
    $pdf->SetFont('Arial','',13);   
    $pdf->Cell(100,7,utf8_decode($areq['depto']), 0, 0,'L');
    ...
    $pdf ->Cell(47,10,utf8_decode('Empresa solicitante '), 1, 0,'L',true);
    $pdf ->SetFont('Arial','',11);
    $pdf->Cell (51,10,utf8_decode($areq['empresa_solicitante']),1,1,'R');

    $pdf ->SetFont('Arial','B',11);
    $pdf ->Cell(47,10,utf8_decode('Centro de costos '), 1, 0,'L',true);
    $pdf ->SetFont('Arial','',11);
    $pdf->Cell (51,10,utf8_decode($areq['centro_costos']),1,1,'R');

    $pdf ->SetFont('Arial','B',11);
    $pdf ->Cell(47,10,utf8_decode('Área operativa '), 1, 0,'L',true);
    $pdf ->SetFont('Arial','',11);
    $pdf->Cell (51,10,utf8_decode($areq['area_operativa']),1,1,'R');

    $pdf ->SetFont('Arial','B',11);
    $pdf ->Cell(47,10,utf8_decode('Tipo de personal '), 1, 0,'L',true);
    $pdf ->SetFont('Arial','',11);
    $pdf->Cell (51,10,utf8_decode($areq['tipo_de_personal']),1,1,'R');

    $pdf ->SetFont('Arial','B',11);
    $pdf ->Cell(47,10,utf8_decode('Puesto solicitado '), 1, 0,'L',true);
    $pdf ->SetFont('Arial','',11);
    $pdf->Cell (51,10,utf8_decode($areq['puesto_solicitado']),1,1,'R');

    $pdf ->SetFont('Arial','B',11);
    $pdf ->Cell(47,10,utf8_decode('Personas requeridas '), 1, 0,'L',true);
    $pdf ->SetFont('Arial','',11);
    $pdf->Cell (51,10,utf8_decode($areq['personas_requeridas']),1,1,'R');

    $pdf ->SetFont('Arial','B',11);
    $pdf ->Cell(47,10,utf8_decode('Grado de estudios '), 1, 0,'L',true);
    $pdf ->SetFont('Arial','',11);
    $pdf->Cell (51,10,utf8_decode($areq['grado_estudios']),1,1,'R');

    $pdf ->SetFont('Arial','B',11);
    $pdf ->Cell(47,10,utf8_decode('Motivo de la requisición'), 1, 0, 'L', true);
    $pdf ->SetFont('Arial','',11);
    $pdf->Cell(51,10,utf8_decode($areq['motivo_requisicion']),1,1,'R');
